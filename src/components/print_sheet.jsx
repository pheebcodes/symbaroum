...
import { races, abilityRanks } from "../data/index.js";
...
function Labeled(props) {
  ...
  return (
    <div
      {...pass}
      className={`labeled ${underlined ? "underlined" : ""} ${className}`}
    >
  ...
  return (
    <div className="info">
      ...
      <div className="center-block">
        <Block label="Maximum Toughness">
          {Math.max(character.get("strong"), 10)}
        </Block>
        <Block label="Current"></Block>
        <Block label="Pain Threshold">
          {Math.ceil(character.get("strong") / 2)}
        </Block>
        <Block label="Corruption Threshold">
          {Math.ceil(character.get("resolute") / 2)}
        </Block>
      ...
      <div className="right-block">
        <Labeled label="Shadow" className="shadow">
          {character.get("shadow")}
        </Labeled>
        <div className="xp">
...
function Ability({ ability }) {
  return (
    <div className="ability">
      <Labeled label="name" underlined={false} className="name">
        {ability.get("name")}
      </Labeled>
      <Labeled label="effect" underlined={false} className="effect">
        {ability.get("effect")}
      </Labeled>
      <div className="type-rank">
        <Labeled label="type" underlined={false}>
          {ability.get("type")}
        </Labeled>
        <Labeled label="rank" underlined={false}>
          {abilityRanks[ability.get("rank")]}
        </Labeled>
      </div>
    </div>
  );
...
function Abilities({ character }) {
  const abilities = listPad(character.get("abilities"), 12)
    .map((e, i) => <Ability key={i} ability={e} />)
    .toJSON();
  return (
...
function Weapon({ weapon }) {
  return (
    <div className="weapon">
      <Labeled className="name" label="weapon">
        {weapon.get("name")}
      </Labeled>
      <Labeled className="damage" label="damage">
        {weapon.get("damage")}
      </Labeled>
      <Labeled className="quality" label="quality">
        {weapon.get("quality")}
      </Labeled>
      <Labeled className="attribute" label="attribute">
        {weapon.get("attribute")}
      </Labeled>
    </div>
  );
}
...
function Armor({ armor }) {
  return (
    <div className="armor">
      <Labeled label="armor" underlined={false}>
        {armor.get("name")}
      </Labeled>
      <Labeled label="protection" underlined={false}>
        {armor.get("protection")}
      </Labeled>
      <Labeled label="quality" underlined={false}>
        {armor.get("quality")}
      </Labeled>
      <div className="defense">
        <Labeled label="defense" underlined={false}>
          {armor.get("defense")}
        </Labeled>
      </div>
    </div>
...
function Equipment({ character }) {
  const weapons = listPad(character.get("weapons"), 4).map((e, i) => (
    <Weapon key={i} weapon={e} />
  ));
  const armor = listPad(character.get("armor"), 2).map((e, i) => (
    <Armor key={i} armor={e} />
  ));
  return (
    <div className="equipment-container container">
      ...
      <div className="equipment">
        ...
        <div className="armors">{armor}</div>
      </div>
    </div>
  );
}
...
function Group({ group }) {
  const members = listPad(group.get("members"), 5).map((e, i) => (
...
function Artifact({ artifact }) {
  return (
    <div className="artifact">
      ...
      <Labeled label="power" className="power">
        {artifact.get("power")}
      </Labeled>
...
function Artifacts({ artifacts }) {
  const elements = listPad(artifacts, 4).map((e, i) => (
    <Artifact key={i} artifact={e} />
  ));
  return (
    <div className="artifact-container container">
      <h2>Artifacts &amp; Mystical Treasures</h2>
      <div className="artifacts">{elements}</div>
    </div>
  );
}
...
function Inventory({ inventory, money, assets }) {
  const items = listPad(inventory, 12).map((e, i) => <Item key={i} item={e} />);
  return (
    <div className="inventory-container container">
      <h2>Inventory</h2>
      <div className="inventory">
        {items}
        <Labeled label="money" className="money">
          {money}
        </Labeled>
        <Labeled label="assets" className="assets">
          {assets}
        </Labeled>
      </div>
    </div>
    ...
    <div className="print">
      <div className="header-container">
        <div className="header">
          ...
          <Link to={`/character/${match.params.id}`} className="button leave">
            Return to Character
          </Link>
        </div>
      </div>
