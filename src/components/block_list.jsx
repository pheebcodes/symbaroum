...
export function SimpleBlockList(Component, defaultElement, opts = {}) {
  ...
  return (props) => {
    const { list, onClick = () => {}, onChange = () => {}, ...pass } = props;
    return (
      <List
        {...pass}
        onChange={(i, v) => onChange(list.set(i, v))}
  ...
  return (props) => {
    ...
    const elements = list.map((e, i) => (
      <div
        ...
        key={i}
        className={classNames("block-list-item", itemClass)}
      >
        <Component
          {...pass}
          value={e}
          className="block-list-component"
          index={i}
    ...
    return (
      <div className={classNames("block-list", className, optsClassName)}>
        {elements.toArray()}
        {list.size < limit && (
          <New label={addLabel} onClick={() => onClickNew()} />
        )}
      </div>
    );
  };
